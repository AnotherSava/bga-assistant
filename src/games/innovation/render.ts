...
import { type CardInfo, type Card, type AgeSetKey, CardSet, CardDatabase, colorLabel, ageSetKey, parseAgeSetKey, cardIndex } from "./types.js";
...
/** Render Innovation turn history as individually keyed rows, binding the card database into
 *  the detail formatter the shared renderer calls. */
export function renderTurnHistoryRows(actions: TurnAction[], cardDb: CardDatabase, players: PlayerInfo[], options: TurnHistoryOptions = {}): TurnHistoryRow[] {
  return renderSharedTurnHistoryRows(actions, players, (detail, popoverTips) => formatActionDetail(detail, cardDb, popoverTips), options);
}
