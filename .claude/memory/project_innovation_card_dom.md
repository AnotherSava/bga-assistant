...
**Layout is JS-computed, not CSS.** `gameui.card_dimensions["M card"]` (182×126, "CSS + 2" for the border), `gameui.delta.my_hand`, `gameui.overlap_for_splay["M card"]` are the constants everything derives from; BGA writes inline `left`/`top` per card and recomputes on every move, splay and resize. Patch those three and call `gameui.refreshLayout()` — it re-runs hand sizing and `refreshSplay` over every pile, so splay direction, echo-effect visibility and pile-width clamping keep working. Hands and boards use `"M card"`; opponents' hands, score, achievements, relics and decks all share `"S recto"` — one entry behind six zone kinds, so it cannot be patched to resize any one of them. See [[project_innovation_zone_placement]] for the lever that does reach a single zone.

**A face-down card carries its age and set, and an id that outlives every move.** `createAndAddToZone(zone, i, age, type, is_relic, id = null, …)` mints a synthetic id — `((uniqueId() * 1000 + age) * 5 + type) * 2 + is_relic`, so it never collides with a real card id — and the element ends up `item_<synthetic>__age_<n>__type_<t>__is_relic_<0|1>__S__recto` with classes `age_<n> type_<t> S recto`. That id is minted once and survives every later move: a hidden→hidden transfer (deck→hand, hand→deck) *reparents the same node* rather than building a new one, so it is the stable sort key for matching per-card knowledge onto anonymous cards. The card has **no children at all** — no name, no age, no icons — except a `.card_back_text` span holding the age when BGA's own preference 110 is set to 2 (`simplified_card_back`), and preference 111 adds a `simplified` class. Key selectors on `.recto`, never on the graphics class, since the user's preferences swap it. Relics are the exception: they use the real card id (`212 + age`) and get BGA's own tooltip.

**Set numbering differs from ours.** BGA's `type_<t>` is 0=base, 1=artifacts, 2=cities, 3=echoes, 4=figures, while our `CardSet` enum is 0=base, 1=figures, 2=cities, 3=echoes, 4=artifacts. Anything matching our cards against BGA's markup must convert (`BGA_SET_ID` in `process_log.ts`).
