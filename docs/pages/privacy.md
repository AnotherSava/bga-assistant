...
- When the optional compact header option is enabled, the extension moves BGA's own header elements into a single row and trims the bar around them. Nothing is copied or re-rendered — BGA keeps updating its own elements — and turning the option off puts every element back where BGA had it.
- When the optional compact player panels option is enabled for Nucleum, the extension restyles BGA's own player panels so their resource counters fold onto one line. This changes only what you see; nothing is sent anywhere, and turning the option off restores the page.
- When the optional police-line highlight is enabled for Innovation, the extension reads whose turn it is from the game page's own live state and stripes BGA's top bar while you must act during another player's turn. That reading happens on your device and stays there; nothing is sent anywhere, and turning the option off restores the page.
